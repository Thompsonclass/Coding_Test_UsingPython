...
#예를 들어 프로세스 4개 [A, B, C, D]가 순서대로 실행 대기 큐에 들어있고, 우선순위가 [2, 1, 3, 2]라면
#[C, D, A, B] 순으로 실행하게 됩니다.
...
def solution(priorities, location):
    ...
    while queue:
        priority, idx = queue.pop(0)  # 리스트에서 프로세스 꺼내기, [(2, 0), (1, 1), (3, 2), (2, 3)]
        # 비워질 때까지 실행, #(요소, 인덱스)
        if any(priority < p for p, _ in queue):  # 더 높은 중요도의 프로세스가 있다면 다시 리스트에 넣기
            queue.append((priority, idx))
        else:  # 현재 프로세스를 실행하고 실행 순서 +1
            answer += 1
            if idx == location:  # 찾고자 하는 프로세스의 위치라면 결과 반환
                return answer
